...
* 👤 **[개발자 성함 최초 식별 수칙]**: LLM 에이전트(개발자)가 사용자와 세션을 시작하거나 새로운 작업을 진행할 때, **"작업을 진행하실 개발자(담당자) 성함/닉네임이 어떻게 되십니까?"**라고 물어보고 식별한 뒤 ADR 문서 작성자로 사용합니다.
...
## 📄 11. AI 대화 기반 기술 의사결정 기록 (Architecture Decision Records - ADR)

1. **개발자 성함 사전 식별 및 기록 수칙**:
   * LLM 에이전트는 세션 시작 시 **작업을 진행하시는 개발자(담당자)의 성함/닉네임**을 확인하여 `docs/adr/` 문서 및 Commit/PR에 수반하여 기록합니다.
2. **기술 의사결정 배경 문서화**:
   * 아키텍처 선택, DB 통합, 수집 스캔 조건 변경 등 주요 결정 발생 시 `docs/adr/000X-feature-name.md` 문서를 템플릿(`docs/adr/0000-template.md`)에 맞추어 자동 작성합니다.
3. **AI 제안 & 인간 최종 승인 주체 명시**:
   * 모든 ADR 문서 및 PR 명세에 "이 아키텍처는 **AI 에이전트의 제안**을 바탕으로, **[개발자 성함] 작성** 및 **[대표님/담당자]의 최종 승인**으로 채택되었다"는 결정 주체를 명시합니다.

---

## 🎯 12. 바이브코딩 컨텍스트 템플릿 & 4단계 요구사항 전달 수칙 (Context & 4-Step Requirements)

### 1) 컨텍스트 템플릿 우선 법칙 (Context-First)
* 프롬프트 문구에 의존하지 않고, 프로젝트의 기술 스택(Next.js 15, Django 5.1, PostgreSQL), 단일 전사 UI 표준(`operations/basic/contracts`), 시큐어 코딩 지침(`AGENTS.md` 및 `docs/`) 등 팀의 암묵지가 압축된 컨텍스트를 최우선 참조합니다.

### 2) 요구사항 4단계 구조화 수칙 (배경–목표–제약–예시)
개발자가 AI 에이전트에 기능 개발/수정을 요청할 때는 아래 4가지 항목을 구조화하여 전달합니다:
* **배경 (Background)**: 어떤 서비스인지, 대상 유저가 누구인지
* **목표 (Goal)**: 이번에 만들고 싶은 기능의 목적
* **제약 (Constraints)**: 사용하는 라이브러리, 인프라, 보안/성능 조건 등
* **예시 (Examples)**: 기존 유사 코드, UI 캡처, API 스펙 등

### 3) ⚠️ 누락 시 AI 에이전트 역질문 수칙 (Strict Requirement Check)
* **개발자(사용자)가 위의 4가지 사항(배경-목표-제약-예시)을 충분히 제시하지 않고 단순 요청만 전달할 경우, LLM 에이전트는 즉시 코드를 수정하지 않고 먼저 '배경, 목표, 제약, 예시' 항목을 보완해 달라고 정중히 역질문/안내한 뒤 작업을 진행합니다.**
