...
12. **`monitoring_data_records` (MonitoringDataRecord 모델 - Cold Time-Series DB)**: 일별 시계열 이력 누적 적재 (`unique_together = ("monitoring_printer", "yyyymmdd")`)
13. **`supplies` (SuppliesAlert 모델)**: 소모품 경고 상태 (`unique_together = ("workplace", "serial_no")`)
14. **`unregistered_printers` (UnregisteredPrinter 모델)**: 현장 탐지 미등록 복합기 저장소 (`unique_together = ("workplace", "ip")`)
