...
| **TC-102** | 파트너관리자/하위메뉴전면제거및2대핵심메뉴재편탑재 | 파트너 관리자 메뉴 기존 5개 하위 항목 제거 후 '파트너 관리(파트너 목록)' 및 '대시보드(모든 파트너 사들의 사용현황)' 2대 메뉴로 재편 신설 | Restructure Partner Admin Submenus to 2 Dedicated Core Views | ✅ PASS |
| **TC-103** | 장비현황/행클릭상세모달도입및목록수정삭제버튼삭제 | `devices` 목록 테이블 액션 버튼 삭제 및 행(Row) 클릭 시 상세 정보 모달 팝업 및 내부 수정/삭제 전환 UX 적용 완수 | Enable Row Click Detail Modal and Remove Action Buttons in Devices Page | ✅ PASS |
